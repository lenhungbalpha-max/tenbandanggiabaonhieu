module.exports = async function handler(req, res) {
  ...
  const searchCode = (code || 'TLCG').toUpperCase();
  const expectedAmount = Number(expectedAmountRaw) || 999000;
  const productName = product || 'Tạo Trợ Lý ChatGPT Cho Mọi Ngành Nghề';
  ...
  try {
    ...
    if (match) {
      ...
      return res.status(200).json({
        success: true,
        message: '✅ Thanh toán thành công! Kết nối Zalo để được thêm vào nhóm hỗ trợ và nhận link khóa học ngay.',
      });
    }
