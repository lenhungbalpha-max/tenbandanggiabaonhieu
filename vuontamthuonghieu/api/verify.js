module.exports = async function handler(req, res) {
  ...
  const searchCode = (code || 'VT').toUpperCase();
  const expectedAmount = Number(expectedAmountRaw) || 499000;
  const productName = product || 'Vườn Tâm Thương Hiệu';
  ...
  try {
    ...
    if (match) {
      ...
      return res.status(200).json({
        success: true,
        message: '✅ Thanh toán thành công! Kết nối Zalo để nhận form cá nhân hoá 3 trợ lý.',
      });
    }
