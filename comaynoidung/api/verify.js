module.exports = async function handler(req, res) {
  ...
  const searchCode = (code || 'CM').toUpperCase();
  const expectedAmount = Number(expectedAmountRaw) || 4999000;
  const productName = product || 'Hệ Thống Cỗ Máy Nội Dung';
  ...
  try {
    ...
    if (match) {
      ...
      return res.status(200).json({
        success: true,
        message: '✅ Thanh toán thành công! Kết nối Zalo để được thêm vào nhóm hỗ trợ và đặt lịch Zoom setup.',
      });
    }
